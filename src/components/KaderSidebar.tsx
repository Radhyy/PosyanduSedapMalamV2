...
export default function KaderSidebar() {
  ...
  const menuItems = [
    { name: "Dashboard Kader", href: "/kader", icon: "fa-table-cells-large" },
    { name: "Data Balita", href: "/kader/balita", icon: "fa-baby" },
    { name: "Input Imunisasi", href: "/kader/imunisasi", icon: "fa-syringe" },
    { name: "Input Pengukuran", href: "/kader/pengukuran", icon: "fa-scale-balanced" },
    { name: "Laporan & Grafik", href: "/kader/laporan", icon: "fa-chart-pie" },
  ];

  return (
    <aside className={`relative bg-white border-r border-gray-100 hidden md:flex flex-col py-8 font-sans transition-all duration-300 ease-in-out ${isCollapsed ? 'w-[90px]' : 'w-[280px]'} px-4 shrink-0`}>
      {/* Toggle Button */}
      <button 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gray-100 flex items-center justify-center overflow-hidden border border-gray-200 shrink-0">
                <img src="https://api.dicebear.com/7.x/notionists/svg?seed=Kader&backgroundColor=f3f4f6" alt="Kader" width={40} height={40} />
              </div>
              {!isCollapsed && (
                <div className="whitespace-nowrap">
                  <span className="text-[15px] font-bold text-gray-900 block">Kader Bidan</span>
                  <span className="text-[11px] text-gray-400 font-medium uppercase tracking-wider block">Kader Posyandu</span>
                </div>
              )}
            </div>
            {!isCollapsed && (
